                              ...
                              return Padding(
                                ...
                                child: ListTile(
                                  ...
                                  subtitle: Row(
                                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                    children: [
                                      if ((permissionService.hasPermission(Permit.purchaseReturnPriceView.value) && widget.purchaseTransaction != null) ||
                                          (!permissionService.hasPermission(Permit.purchaseReturnPriceView.value) && widget.purchaseTransaction == null))
                                        Text(
                ...
                Expanded(
                  child: ElevatedButton(
                    style: OutlinedButton.styleFrom(
                      maximumSize: const Size(double.infinity, 48),
                      minimumSize: const Size(double.infinity, 48),
                      disabledBackgroundColor: _theme.colorScheme.primary.withAlpha(38),
                    ),
                    onPressed: widget.saleTransactionModel != null
                        ? () async {
                            EasyLoading.show();
                            returnList.removeWhere((element) => element.quantity < 0.01);

                            if (returnList.isNotEmpty) {
                              if (!permissionService.hasPermission(Permit.saleReturnsCreate.value)) {
                                EasyLoading.dismiss();
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    backgroundColor: Colors.red,
                                    content: Text('You do not have permission to create sale returns.'),
                                  ),
                                );
                                return;
                              }
                              ...
                              try {
                                ...
                                for (var items in returnList) {
                                  ...
                                  data.lossProfit.add((items.lossProfit! / items.stock!) * items.quantity);
                                }
                                ...
                                if (result ?? false) {
                                  ...
                                  EasyLoading.showError('Failed to process sale return.');
                                }
                              } catch (e) {
                                EasyLoading.dismiss();
                        ...
                        : () async {
                            ...
                            if (returnList.isNotEmpty) {
                              if (!permissionService.hasPermission(Permit.purchaseReturnsCreate.value)) {
                                EasyLoading.dismiss();
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    backgroundColor: Colors.red,
                                    content: Text('You do not have permission to create purchase returns.'),
                                  ),
                                );
                                return;
                              }
                              ...
                              try {
                                ...
                                if (result ?? false) {
                                  ...
                                  EasyLoading.showError('Failed to process purchase return.');
                                }
                              } catch (e) {
                                EasyLoading.dismiss();
                                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.toString())));
